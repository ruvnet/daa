...
use crate::{
    ...
    server::DaaMcpServer,
    swarm::{SwarmConfig, SwarmCoordinator, SwarmMode, SwarmStrategy, SwarmTemplates},
    tools::execute_tool,
...
impl DaaIntegrationManager {
    ...
    pub async fn new(mcp_config: DaaMcpConfig, discovery_config: DiscoveryConfig) -> Result<Self> {
        ...
        let discovery =
            Arc::new(DiscoveryProtocol::new(discovery_config, server_state.clone()).await?);

        // Initialize swarm coordinator
        let swarm_coordinator =
            Arc::new(SwarmCoordinator::new(server_state.clone(), discovery.clone()).await?);
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        let discovery_mut = Arc::get_mut(&mut self.discovery).ok_or_else(|| {
            DaaMcpError::Protocol("Cannot get mutable reference to discovery".to_string())
        })?;
        discovery_mut.start().await?;
        info!("Discovery protocol started");

        // Start swarm coordinator
        let swarm_mut = Arc::get_mut(&mut self.swarm_coordinator).ok_or_else(|| {
            DaaMcpError::Protocol("Cannot get mutable reference to swarm coordinator".to_string())
        })?;
    ...
    pub async fn execute_3_agent_research_swarm(&self, research_objective: &str) -> Result<String> {
        info!(
            "Executing 3-agent research swarm for: {}",
            research_objective
        );
        ...
        let swarm_id = self
            .swarm_coordinator
            .create_swarm(swarm_config, required_agent_types)
            .await?;
        ...
        for task in tasks {
            self.swarm_coordinator
                .add_swarm_task(&swarm_id, task)
                .await?;
        }

        // Step 5: Monitor execution and wait for completion
        let results = self
            .monitor_swarm_execution(&swarm_id, Duration::from_secs(300))
            .await?;

        info!(
            "3-agent research swarm completed with {} results",
            results.len()
        );
        Ok(format!(
            "Research swarm {} completed successfully with {} results",
            swarm_id,
            results.len()
        ))
    }
    ...
    pub async fn execute_3_agent_development_swarm(
        &self,
        development_objective: &str,
    ) -> Result<String> {
        info!(
            "Executing 3-agent development swarm for: {}",
            development_objective
        );

        // Create development swarm
        let swarm_config =
            SwarmTemplates::development_swarm_3_agent(development_objective.to_string());
        let required_agent_types = vec![
            "coder".to_string(),
            "tester".to_string(),
            "reviewer".to_string(),
        ...
        let results = self
            .monitor_swarm_execution(&swarm_id, Duration::from_secs(600))
            .await?;

        Ok(format!(
            "Development swarm {} completed with {} deliverables",
            swarm_id,
            results.len()
        ))
    }
    ...
    pub async fn execute_3_agent_analysis_swarm(&self, analysis_objective: &str) -> Result<String> {
        info!(
            "Executing 3-agent analysis swarm for: {}",
            analysis_objective
        );
        ...
        let tasks = self
            .create_analysis_tasks_with_dependencies(analysis_objective)
            .await?;
        ...
        Ok(format!(
            "Analysis swarm {} generated comprehensive report with {} insights",
            swarm_id,
            results.len()
        ))
    }
    ...
    pub async fn demonstrate_parallel_batch_execution(&self) -> Result<Vec<String>> {
        ...
        let batch_tasks = vec![
            ...
            (
                "discover_agents",
                json!({"required_capabilities": ["analysis", "research"]}),
            ),
            ...
            .map(|(tool_name, args)| {
                ...
                async move {
                    match timeout(
                        ...
                        execute_tool(state, tool_name, args),
                    )
                    .await
                    {
    ...
    async fn create_analysis_tasks_with_dependencies(
        &self,
        objective: &str,
    ) -> Result<Vec<DaaTask>> {
    ...
    async fn monitor_swarm_execution(
        &self,
        swarm_id: &str,
        ...
        loop {
            ...
            if start_time.elapsed().unwrap_or_default() > timeout_duration {
                warn!(
                    "Swarm execution monitoring timed out for swarm: {}",
                    swarm_id
                );
                break;
            }
            ...
            match self.swarm_coordinator.get_swarm_status(swarm_id).await {
                Ok(swarm_state) => {
                    ...
                    info!(
                        "Swarm {} status: {} completed, {} active, {} pending",
    ...
    async fn test_swarm_coordination(&self) -> Result<()> {
        ...
        match self
            .swarm_coordinator
            .create_swarm(test_config, required_types)
            .await
        {
            Ok(_) => Ok(()),
