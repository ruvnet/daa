...
use crate::{
    ClientCapabilities, ClientInfo, DaaMcpError, InitializeRequest, InitializeResponse, McpMessage,
    Result, ToolCall, ToolResult, MCP_PROTOCOL_VERSION,
};
...
impl DaaMcpClient {
    ...
    pub async fn initialize(&mut self) -> Result<InitializeResponse> {
        ...
        if let Some(result) = response.result {
            ...
            info!(
                "Successfully initialized MCP connection to {}",
                self.endpoint
            );
            Ok(init_response)
    ...
    pub async fn list_resources(&self) -> Result<Vec<crate::ResourceInfo>> {
        ...
        if let Some(result) = response.result {
            let resources_result = result.get("resources").ok_or_else(|| {
                DaaMcpError::Protocol("Missing resources in response".to_string())
            })?;
            let resources: Vec<crate::ResourceInfo> =
                serde_json::from_value(resources_result.clone())?;
            Ok(resources)
        } else if let Some(error) = response.error {
            Err(DaaMcpError::Protocol(format!(
            ...
            Err(DaaMcpError::Protocol(
                "Invalid resources list response".to_string(),
            ))
        }
    }
    ...
    pub async fn read_resource(&self, uri: &str) -> Result<Vec<crate::Content>> {
        ...
        if let Some(result) = response.result {
            ...
            Err(DaaMcpError::Protocol(
                "Invalid resource read response".to_string(),
            ))
        }
    }
    ...
    pub async fn list_prompts(&self) -> Result<Vec<crate::prompts::PromptInfo>> {
        ...
        if let Some(result) = response.result {
            let prompts_result = result
                .get("prompts")
                .ok_or_else(|| DaaMcpError::Protocol("Missing prompts in response".to_string()))?;
            let prompts: Vec<crate::prompts::PromptInfo> =
                serde_json::from_value(prompts_result.clone())?;
            Ok(prompts)
        } else if let Some(error) = response.error {
            Err(DaaMcpError::Protocol(format!(
            ...
            Err(DaaMcpError::Protocol(
                "Invalid prompts list response".to_string(),
            ))
        }
    }
    ...
    pub async fn get_prompt(
        ...
        name: &str,
        arguments: Value,
    ) -> Result<crate::prompts::PromptResponse> {
        let message = McpMessage {
            jsonrpc: "2.0".to_string(),
            id: Some(serde_json::Value::Number(
                self.next_request_id().await.into(),
...
impl DaaOperationsClient {
    ...
    pub async fn spawn_agent(&self, config: crate::AgentConfig) -> Result<String> {
        ...
        Err(DaaMcpError::Protocol(
            "Failed to extract agent ID from response".to_string(),
        ))
    }
    ...
    pub async fn list_agents(&self) -> Result<Vec<crate::DaaAgentInfo>> {
        let result = self
            .client
            .call_tool("list_agents", serde_json::json!({}))
            .await?;
            ...
            if let Some(first_content) = content.first() {
                if first_content.content_type == "application/json" {
                    ...
                    if let Some(agents_array) = data.get("agents") {
                        let agents: Vec<crate::DaaAgentInfo> =
                            serde_json::from_value(agents_array.clone())?;
                        return Ok(agents);
                    }
                }
            }
        ...
        Err(DaaMcpError::Protocol(
            "Failed to parse agents list".to_string(),
        ))
    }
    ...
    pub async fn create_task(&self, task: crate::DaaTask) -> Result<String> {
        ...
        Err(DaaMcpError::Protocol(
            "Failed to extract task ID from response".to_string(),
        ))
    }
    ...
    pub async fn coordinate_swarm(
        ...
        Err(DaaMcpError::Protocol(
            "Failed to extract swarm ID from response".to_string(),
        ))
    }
    ...
    pub async fn get_system_metrics(&self) -> Result<serde_json::Value> {
        let result = self
            .client
            .call_tool("get_system_metrics", serde_json::json!({}))
            .await?;
        ...
        Err(DaaMcpError::Protocol(
            "Failed to parse system metrics".to_string(),
        ))
    }
    ...
    pub async fn health_check(&self, deep_check: bool) -> Result<serde_json::Value> {
        ...
        Err(DaaMcpError::Protocol(
            "Failed to parse health check result".to_string(),
        ))
    }
