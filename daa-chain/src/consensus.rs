...
use crate::{ChainConfig, ChainError, Result};
...
pub enum ConsensusEvent {
    ...
    /// Vote received
    VoteReceived { voter: String, vote: Vote },
    ...
    /// Round timeout
    RoundTimeout { epoch: u64, round: u64 },
...
impl ConsensusEngine {
    ...
    pub async fn new(config: &ChainConfig) -> Result<Self> {
        let qudag_consensus = QuDAGConsensus::new()
            .await
    ...
    pub async fn propose_block(&mut self, block: Block) -> Result<()> {
        ...
        state
            .proposals
            .insert("our_node_id".to_string(), block.clone());
            ...
            loop {
                ...
                if Self::should_timeout_round(&state_guard).await {
                    ...
                    if let Some(new_leader) =
                        Self::select_leader(&validators, state_guard.epoch, state_guard.round).await
                    {
    ...
    async fn select_leader(
        ...
        let active_validators: Vec<_> = validators_guard.values().filter(|v| v.is_active).collect();
