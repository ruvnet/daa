...
use daa_rules::{ExecutionContext, Rule, RuleEngine, RuleResult};
...
impl RulesBridge {
    ...
    pub async fn validate_transaction(
        ...
        let applicable_rules =
            self.find_applicable_rules(&RuleTrigger::BeforeTransactionValidation);

        for rule in applicable_rules {
            if self
                .evaluate_conditions(&rule.conditions, transaction, context)
                .await?
            {
                let result = self
                    .execute_actions(&rule.actions, transaction, context)
                    .await?;
    ...
    pub async fn validate_block(
        ...
        for rule in applicable_rules {
            if self
                .evaluate_block_conditions(&rule.conditions, block, context)
                .await?
            {
                let result = self
                    .execute_block_actions(&rule.actions, block, context)
                    .await?;
                results.push((rule.id.clone(), result));
            }
        }
    ...
    pub async fn process_transaction_execution(
        ...
        for rule in applicable_rules {
                ...
                .await?
            {
                ...
                let result = self
                    .rule_engine
                    .execute_rule(&rule_def, &mut execution_context)
                    .await
    ...
    fn find_applicable_rules(&self, trigger: &RuleTrigger) -> Vec<&ChainRule> {
        self.chain_rules
            ...
            .filter(|rule| {
                rule.enabled
                    && std::mem::discriminant(&rule.trigger) == std::mem::discriminant(trigger)
            })
            .collect()
    }
        ...
        for condition in conditions {
            match condition {
                ...
                RuleCondition::Custom(condition_type, params) => {
                    ...
                    let rule_condition =
                        daa_rules::RuleCondition::Custom(condition_type.clone(), params.clone());
                    let mut exec_context = self.create_execution_context(context)?;
                    let result = self
                        .rule_engine
                        .evaluate_condition(&rule_condition, &exec_context)
                        .await
                        .map_err(|e| {
                            ChainError::Storage(format!("Condition evaluation failed: {}", e))
                        })?;
        ...
        for action in actions {
            match action {
                ...
                RuleAction::Custom(action_type, params) => {
                    ...
                    let rule_action =
                        daa_rules::RuleAction::Custom(action_type.clone(), params.clone());
                    let mut exec_context = self.create_execution_context(context)?;
                    self.rule_engine
                        .execute_action(&rule_action, &mut exec_context)
                        .await
                        .map_err(|e| {
                            ChainError::Storage(format!("Action execution failed: {}", e))
                        })?;
                }
            }
        }
    ...
    fn create_execution_context(
        ...
        context.set_variable(
            ...
            chain_context.chain_height.to_string(),
        );
...
impl ValidationResult {
    ...
    pub fn is_allowed(&self) -> bool {
        !self
            .rule_results
            .iter()
            .any(|(_, result)| matches!(result, ActionResult::Deny(_)))
    }
