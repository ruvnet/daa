...
impl GradientCompressor {
    ...
    fn compress_sparse_format(&self, gradient: &[f32]) -> Result<Vec<u8>> {
        ...
        let non_zero_indices: Vec<(u32, f32)> = gradient
            .iter()
            .enumerate()
    ...
    pub fn decompress_sparse(&self, compressed: &[u8]) -> Result<Vec<f32>> {
        ...
        for _ in 0..non_zero_count {
            ...
            let idx = u32::from_le_bytes([
                sparse_data[offset],
                sparse_data[offset + 1],
                sparse_data[offset + 2],
                sparse_data[offset + 3],
            ]) as usize;

            let value = f32::from_le_bytes([
                sparse_data[offset + 4],
                sparse_data[offset + 5],
                sparse_data[offset + 6],
                sparse_data[offset + 7],
            ]);
