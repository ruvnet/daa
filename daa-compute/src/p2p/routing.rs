...
impl Router {
    ...
    fn find_dht_route(&self, destination: &PeerId) -> Result<Vec<PeerId>> {
        ...
        let route: Vec<PeerId> = candidates
            .into_iter()
            .take(3)
    ...
    fn find_onion_route(&self, destination: &PeerId, layers: usize) -> Result<Vec<PeerId>> {
        ...
        let mut available_peers: Vec<_> = table
            .keys()
        ...
        let mut route: Vec<PeerId> = available_peers.into_iter().take(layers).collect();
    ...
    pub fn process_message(&self, message: RoutedMessage) -> Result<Option<Vec<u8>>> {
        ...
        self.pending_messages
            .write()
            .insert(message.id.clone(), forwarded);
...
fn calculate_route_cost(entry: &RouteEntry) -> f64 {
    let latency_cost = entry
        .latency
        .map(|l| l.as_millis() as f64 / 1000.0)
        .unwrap_or(1.0);

    let bandwidth_cost = entry
        .bandwidth
