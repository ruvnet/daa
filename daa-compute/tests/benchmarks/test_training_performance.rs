...
use daa_compute::protocols::aggregation::{AggregationStrategy, GradientAggregator};
...
mod benchmarks {
    ...
    async fn benchmark_gradient_aggregation_small() -> Result<()> {
        ...
        println!(
            "Small gradient aggregation (10 nodes, 1K params): {:?}",
            duration
        );
    ...
    async fn benchmark_gradient_aggregation_medium() -> Result<()> {
        ...
        println!(
            "Medium gradient aggregation (50 nodes, 10K params): {:?}",
            duration
        );
    ...
    async fn benchmark_gradient_aggregation_large() -> Result<()> {
        ...
        println!(
            "Large gradient aggregation (100 nodes, 100K params): {:?}",
            duration
        );
    ...
    async fn benchmark_parallel_aggregation() -> Result<()> {
        ...
        for batch_idx in 0..num_batches {
            let batch = (0..batch_size)
                .map(|i| {
                    create_test_gradient(&format!("batch{}_node{}", batch_idx, i), gradient_size, 1)
                })
                .collect();
            batches.push(batch);
        }
        ...
        println!(
            "Parallel aggregation ({} batches, {} nodes each, {}K params): {:?}",
            num_batches,
            batch_size,
            gradient_size / 1000,
            duration
    ...
    async fn benchmark_network_throughput() -> Result<()> {
        ...
        let data_rate = (num_gradients * gradient_size * 4) as f64
            / send_duration.as_secs_f64()
            / 1024.0
            / 1024.0; // MB/s

        println!(
            "Network throughput: {:.1} gradients/sec, {:.1} MB/s",
    ...
    async fn benchmark_byzantine_resilience() -> Result<()> {
        ...
        println!(
            "Byzantine-resilient aggregation ({} honest, {} Byzantine, {}K params): {:?}",
            honest_nodes,
            byzantine_nodes,
            gradient_size / 1000,
            duration
    ...
    async fn benchmark_memory_usage() -> Result<()> {
        ...
        println!(
            "Estimated gradient memory: {:.1} MB",
            gradient_memory as f64 / 1024.0 / 1024.0
        );
        ...
        println!(
            "Large-scale aggregation memory test ({} nodes, {}M params): {:?}",
            num_nodes,
            large_gradient_size / 1000000,
            duration
        );
    ...
    async fn benchmark_concurrent_aggregation() -> Result<()> {
        ...
        for task_id in 0..num_concurrent {
            let handle = tokio::spawn(async move {
                ...
                let (result, _) = aggregator
                    .aggregate(gradients, task_id as u64)
                    .await
                    .unwrap();
        ...
        println!(
            "Concurrent aggregation ({} tasks, {} nodes each, {}K params): {:?}",
            num_concurrent,
            gradients_per_task,
            gradient_size / 1000,
            total_duration
    ...
    async fn benchmark_compression_accuracy() -> Result<()> {
        ...
        for method in compression_methods {
            ...
            let mse: f32 = original_gradient
                .iter()
                .zip(decompressed.iter())
                .map(|(a, b)| (a - b).powi(2))
                .sum::<f32>()
                / original_gradient.len() as f32;

            let compression_ratio = compressed.len() as f32 / (original_gradient.len() * 4) as f32;

            println!(
                "Compression accuracy - MSE: {:.6}, Ratio: {:.3}, Time: {:?}",
