        ...
        group.bench_with_input(
            ...
            |b, (layers, batch_size, num_nodes)| {
                b.to_async(&rt).iter(|| async move {
                    ...
                    let pytorch_result =
                        simulate_pytorch_training(&layers, *batch_size, *num_nodes).await;
                    ...
                    let comparison = ComparisonResult {
                        ...
                        performance_ratio: daa_result.training_time_ms
                            / pytorch_result.training_time_ms,
                        bandwidth_efficiency_ratio: daa_result.bandwidth_utilization_mbps
                            / pytorch_result.bandwidth_utilization_mbps,
                        memory_efficiency_ratio: daa_result.memory_usage_mb
                            / pytorch_result.memory_usage_mb,
                    };
    ...
    for node_count in node_counts {
        ...
        group.bench_with_input(
            ...
            |b, &node_count| {
                b.to_async(&rt).iter(|| async move {
                    let pytorch_comm =
                        simulate_pytorch_communication(&model_size, node_count).await;
...
fn benchmark_scalability_comparison(c: &mut Criterion) {
    ...
    for (scale_name, node_count, layers) in scale_scenarios {
        group.bench_with_input(
            ...
            |b, (node_count, layers)| {
                b.to_async(&rt).iter(|| async move {
                    let pytorch_scalability =
                        measure_pytorch_scalability(layers, *node_count).await;
...
fn benchmark_fault_tolerance_comparison(c: &mut Criterion) {
    ...
    for (fault_name, fault_percentage) in fault_scenarios {
        group.bench_with_input(
            ...
            |b, &fault_percentage| {
                b.to_async(&rt).iter(|| async move {
                    let pytorch_resilience =
                        simulate_pytorch_with_faults(&model_layers, node_count, fault_percentage)
                            .await;
                    let daa_resilience =
                        simulate_daa_with_faults(&model_layers, node_count, fault_percentage).await;
...
fn benchmark_bandwidth_efficiency_comparison(c: &mut Criterion) {
    ...
    for (bandwidth_name, bandwidth_mbps) in bandwidth_scenarios {
        group.throughput(Throughput::Bytes(
            (bandwidth_mbps * 1024.0 * 1024.0 / 8.0) as u64,
        ));
        ...
        group.bench_with_input(
            ...
            |b, &bandwidth_mbps| {
                b.to_async(&rt).iter(|| async move {
                    let pytorch_efficiency =
                        simulate_pytorch_bandwidth_usage(&model_layers, node_count, bandwidth_mbps)
                            .await;
                    let daa_efficiency =
                        simulate_daa_bandwidth_usage(&model_layers, node_count, bandwidth_mbps)
                            .await;
    ...
    for (task_name, target_loss, expected_epochs) in convergence_scenarios {
        group.bench_with_input(
            ...
            |b, &(target_loss, expected_epochs)| {
                b.to_async(&rt).iter(|| async move {
                    let pytorch_convergence =
                        simulate_pytorch_convergence(&model_layers, node_count, target_loss).await;
                    let daa_convergence =
                        simulate_daa_convergence(&model_layers, node_count, target_loss).await;
    ...
    for (hetero_name, variance) in heterogeneity_levels {
        group.bench_with_input(
            ...
            |b, &variance| {
                b.to_async(&rt).iter(|| async move {
                    let pytorch_perf =
                        simulate_pytorch_heterogeneous(&model_layers, node_count, variance).await;
                    let daa_perf =
                        simulate_daa_heterogeneous(&model_layers, node_count, variance).await;
...
async fn simulate_pytorch_training(
    layers: &[usize],
    batch_size: usize,
    num_nodes: usize,
) -> PyTorchMetrics {
...
async fn simulate_daa_training(
    layers: &[usize],
    batch_size: usize,
    num_nodes: usize,
) -> DaaMetrics {
...
async fn simulate_daa_communication(layers: &[usize], num_nodes: usize) -> f64 {
    ...
    let comm_time =
        param_count as f64 * (local_rounds + global_rounds) * 0.0007 * compression_factor;
...
async fn simulate_pytorch_with_faults(
    layers: &[usize],
    node_count: usize,
    fault_percentage: u32,
) -> f64 {
...
async fn simulate_daa_with_faults(
    layers: &[usize],
    node_count: usize,
    fault_percentage: u32,
) -> f64 {
    ...
    let consensus_penalty = if failed_nodes as f64 / node_count as f64 > 0.33 {
        0.5
    } else {
        0.9
...
async fn simulate_pytorch_bandwidth_usage(
    layers: &[usize],
    node_count: usize,
    bandwidth_mbps: f64,
) -> f64 {
...
async fn simulate_daa_bandwidth_usage(
    layers: &[usize],
    node_count: usize,
    bandwidth_mbps: f64,
) -> f64 {
...
async fn simulate_pytorch_convergence(
    layers: &[usize],
    node_count: usize,
    target_loss: f64,
) -> (f64, u32) {
...
async fn simulate_daa_convergence(
    layers: &[usize],
    node_count: usize,
    target_loss: f64,
) -> (f64, u32) {
