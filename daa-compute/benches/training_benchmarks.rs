...
impl BenchmarkModel {
    ...
    pub fn compute_gradients(&self, input: &[f32], target: &[f32]) -> (Vec<f32>, Vec<f32>) {
        ...
        let weight_gradients: Vec<f32> = self
            .weights
            .iter()
            .map(|w| loss * 0.001 * w.signum())
            .collect();
        let bias_gradients: Vec<f32> = self
            .biases
            .iter()
        ...
        group.bench_with_input(
            ...
            |b, layers| {
                b.to_async(&rt).iter(|| async move {
                    ...
                    let dataset =
                        generate_training_data(1000, layers[0], layers.last().unwrap().clone());
            ...
            |b, &batch_size| {
                b.to_async(&rt).iter(|| async move {
                    ...
                    let convergence_time =
                        train_with_batch_size(&mut model, &dataset, batch_size).await;
                    black_box(convergence_time)
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, &sync_freq| {
                b.to_async(&rt).iter(|| async move {
                    ...
                    let start = std::time::Instant::now();
                    let convergence_time =
                        train_with_sync_frequency(&model_layers, &dataset, node_count, sync_freq)
                            .await;
                    black_box(convergence_time)
                });
            },
...
fn benchmark_communication_overhead(c: &mut Criterion) {
    ...
    let model_layers = vec![784, 512, 256, 10];
    let gradient_size: usize = model_layers
        .windows(2)
        .map(|pair| pair[0] * pair[1])
        .sum::<usize>()
        + model_layers[1..].iter().sum::<usize>();
        ...
        group.bench_with_input(
            ...
            |b, &(pattern, node_count)| {
                b.to_async(&rt).iter(|| async move {
                    ...
                    let start = std::time::Instant::now();
                    let comm_time =
                        simulate_gradient_communication(pattern, &gradients, node_count).await;
                    black_box(comm_time)
                });
            },
        );
...
async fn simulate_gradient_communication(
    pattern: &str,
    gradients: &[f32],
    node_count: usize,
) -> Duration {
    ...
    match pattern {
        ...
        "ring_all_reduce" => {
            ...
            for _ in 0..steps {
                tokio::time::sleep(Duration::from_micros(
                    (gradient_size / node_count / 1000) as u64,
                ))
                .await;
            }
        }
        "hierarchical" => {
            ...
            tokio::time::sleep(Duration::from_micros((gradient_size / 4000) as u64)).await;
            // Global
        }
        _ => {
            tokio::time::sleep(Duration::from_micros((gradient_size / 1000) as u64)).await;
...
async fn simulate_federated_learning(
    ...
    for round in 0..rounds {
        ...
        let client_handles: Vec<_> = (0..participating_clients)
            .map(|_| {
                let layers = layers.to_vec();
                tokio::spawn(async move {
                    // Local training
                    let local_dataset = generate_training_data(
                        local_dataset_size,
                        layers[0],
                        *layers.last().unwrap(),
                    );
        ...
        let aggregation_time =
            Duration::from_millis((param_count * participating_clients) as u64 / 10000);
