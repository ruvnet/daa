...
impl ResourceMonitor {
    ...
    pub fn get_metrics(&self) -> ResourceMetrics {
        ...
        ResourceMetrics {
            ...
            gpu_usage_percent: if gpu_operations > 0 {
                Some((gpu_operations as f64 / elapsed) / 10000.0)
            } else {
                None
            },
            gpu_memory_mb: if gpu_operations > 0 {
                Some(memory_bytes as f64 / (1024.0 * 1024.0) * 0.5)
            } else {
                None
            },
...
fn benchmark_network_latency_consensus(c: &mut Criterion) {
    ...
    for (latency_name, latency_ms) in latency_scenarios {
        for validator_count in &validator_counts {
            group.bench_with_input(
                BenchmarkId::new(
                    format!("latency_{}_validators", latency_name),
                    validator_count,
                ),
                ...
                |b, &(latency_ms, validator_count)| {
                    b.to_async(&rt).iter(|| async move {
                        ...
                        simulate_consensus_with_latency(&monitor, validator_count, latency_ms)
                            .await;
