...
use daa_orchestrator::{
    ...
    config::{AiConfig, AutonomyConfig, HealthCheckConfig, RulesConfig},
    services::Service,
...
async fn test_basic_health_checks() {
    ...
    assert!(
        !initial_stats.node_id.is_empty(),
        "Node should be healthy after initialization"
    );
...
async fn test_health_monitoring_edge_cases() {
    ...
    assert!(
        !stats.node_id.is_empty(),
        "Should handle extreme health check config"
    );
    ...
    for i in 0..20 {
        ...
        assert!(
            !stats.node_id.is_empty(),
            "Should handle rapid health checks {}",
            i
        );
...
async fn test_component_monitoring() {
    ...
    println!(
        "   Registered Services: {}",
        initial_stats.registered_services
    );
    println!(
        "   Coordinated Operations: {}",
        initial_stats.coordinated_operations
    );
    ...
    let workflow_result = orchestrator
        .execute_workflow(monitoring_workflow)
        .await
        .unwrap();
    ...
    assert_eq!(
        final_stats.node_id, initial_stats.node_id,
        "Node ID should remain consistent"
    );
...
async fn test_system_recovery_scenarios() {
    ...
    for round in 0..3 {
        ...
        assert!(
            ...
            "Should recover after stress round {}",
            round
        );
        assert_eq!(
            recovery_stats.node_id, baseline_stats.node_id,
            "Node ID should remain consistent"
        );
    ...
    assert!(
        !final_stats.node_id.is_empty(),
        "Should maintain health after all recovery tests"
    );
...
async fn test_monitoring_autonomy_integration() {
    ...
    println!(
        "   Health percentage: {:.1}%",
        (healthy_samples as f64 / total_samples as f64) * 100.0
    );

    // Verify health statistics
    assert!(total_samples > 10, "Should collect multiple health samples");
    assert!(
        healthy_samples > total_samples / 2,
        "Should be healthy for majority of time"
    );
