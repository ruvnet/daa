...
use daa_orchestrator::{
    config::{ApiConfig, AutonomyConfig, ExchangeConfig, McpConfig, QuDAGConfig},
...
async fn test_service_registry_integration() {
    ...
    let rules_engines = orchestrator
        .discover_services("rules_engine")
        .await
        .unwrap();
    let blockchain_bridges = orchestrator
        .discover_services("blockchain_bridge")
        .await
        .unwrap();

    // Test service discovery with non-existent type
    let unknown_services = orchestrator
        .discover_services("unknown_type")
        .await
        .unwrap();
    assert!(
        unknown_services.is_empty(),
        "Should return empty for unknown service types"
    );
...
async fn test_qudag_protocol_integration() {
    let config = OrchestratorConfig {
        qudag: QuDAGConfig {
            ...
            bootstrap_peers: vec!["localhost:7101".to_string(), "localhost:7102".to_string()],
    ...
    assert!(
        init_result.is_ok(),
        "Failed to initialize with QuDAG integration"
    );
    ...
    assert!(
        ...
        "Node ID should be set after QuDAG initialization"
    );
...
async fn test_mcp_server_integration() {
    ...
    assert!(
        !stats.node_id.is_empty(),
        "Orchestrator should be operational with MCP"
    );
...
async fn test_api_server_integration() {
    ...
    assert!(
        !stats.node_id.is_empty(),
        "Orchestrator should be operational with API server"
    );
...
async fn test_event_management_integration() {
    ...
    assert!(
        result.is_ok(),
        "Failed to execute workflow for event integration"
    );
...
async fn test_full_integration_scenario() {
    ...
    assert!(
        ...
        "Full integration config should be valid"
    );
    ...
    assert!(
        init_result.is_ok(),
        "Failed to initialize full integration orchestrator"
    );
...
async fn test_integration_error_handling() {
    ...
    assert!(
        init_result.is_ok(),
        "Should handle partial integration failures gracefully"
    );
    ...
    assert!(
        ...
        "Service registration should work despite partial failures"
    );
...
async fn test_integration_scaling() {
    ...
    println!(
        "Found {} AI agents, {} rules engines, {} blockchain bridges",
        ai_agents.len(),
        rules_engines.len(),
        blockchain_bridges.len()
    );

    // Should complete reasonably quickly even with many services
    assert!(
        registration_time < Duration::from_secs(5),
        "Service registration took too long"
    );
    assert!(
        discovery_time < Duration::from_secs(10),
        "Service discovery took too long"
    );
}
