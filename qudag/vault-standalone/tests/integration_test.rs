...
fn test_vault_create_and_operations() {
    ...
    // Test adding secrets
    vault.add_secret("email/work", "user@example.com", Some("password123")).unwrap();
    vault.add_secret("social/github", "myusername", None).unwrap(); // Generated password
    ...
    assert!(matches!(result, Err(VaultError::InvalidPassword) | Err(VaultError::Crypto(_))));
}
...
fn test_vault_export_import() {
    ...
    vault1.add_secret("test/secret1", "user1", Some("pass1")).unwrap();
    vault1.add_secret("test/secret2", "user2", Some("pass2")).unwrap();
...
fn test_vault_password_generation() {
    ...
    assert_ne!(pass1, vault.generate_password(16, CharacterSet::Alphanumeric));
}