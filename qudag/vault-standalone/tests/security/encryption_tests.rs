...
mod encryption_security_tests {
    ...
    fn test_vault_key_derivation_strength() {
        ...
        assert!(duration > Duration::from_millis(100), 
                "Key derivation too fast, may be insecure: {:?}", duration);
        
        // But not too slow for usability
        assert!(duration < Duration::from_secs(5), 
                "Key derivation too slow: {:?}", duration);
    }
    ...
    fn test_encrypted_vault_file_content() {
        ...
        vault.add_secret("test/secret", "testuser", Some("MySecretPassword")).unwrap();
        drop(vault);
        ...
        assert!(!vault_str.contains("MySecretPassword"), "Password found in plaintext!");
        assert!(!vault_str.contains("testuser"), "Username found in plaintext!");
        assert!(!vault_str.contains("test/secret"), "Label found in plaintext!");
        assert!(!vault_str.contains(password), "Master password found in plaintext!");
    }
    ...
    fn test_memory_zeroization() {
        ...
        // Test that sensitive data is zeroized
        {
            ...
            unsafe {
                ...
                assert!(slice.iter().all(|&b| b == 0), "Password memory not zeroized!");
            }
        }
    }
    ...
    fn test_timing_attack_resistance() {
        ...
        assert!(max_deviation < twenty_percent,
                "Timing attack possible: max deviation {:?} exceeds 20% of avg {:?}",
                max_deviation, avg_duration);
    }
}
...
mod quantum_resistance_tests {
    use super::*;
    use qudag_crypto::{KyberKeyPair, DilithiumKeyPair};
    ...
    fn test_kyber_key_encapsulation() {
        ...
        let encapsulated = vault.export_vault_key_for(&recipient_keypair.public()).unwrap();
        
        // Verify encapsulation size is correct for Kyber
        assert!(encapsulated.ciphertext.len() > 1000, "Kyber ciphertext too small");
        assert!(encapsulated.ciphertext.len() < 2000, "Kyber ciphertext too large");
    }
    ...
    fn test_dilithium_signature_verification() {
        ...
        let mut vault = Vault::create(vault_path.to_str().unwrap(), "TestPassword").unwrap();
        vault.add_secret("test/secret", "user", Some("pass")).unwrap();
    ...
    fn test_quantum_safe_password_generation() {
        ...
        assert!(entropy > 4.0, "Generated password has low entropy: {}", entropy);
    }
}
...
mod side_channel_tests {
    ...
    fn test_constant_time_comparison() {
        ...
        assert!(max_deviation < avg_timing / 10,
                "Timing not constant: max deviation {:?}", max_deviation);
    }
    ...
    fn test_cache_timing_resistance() {
        ...
        for i in 0..1000 {
            vault.add_secret(&format!("test/secret{}", i), "user", Some("pass")).unwrap();
        }
        
        // Access patterns should not reveal information through cache timing
        let labels = vec!["test/secret0", "test/secret500", "test/secret999", "nonexistent"];
        let mut timings = Vec::new();
        ...
        for timing in &timings {
            let deviation = if *timing > avg_timing { *timing - avg_timing } else { avg_timing - *timing };
            assert!(deviation < avg_timing / 2,
