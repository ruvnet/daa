...
use qudag_crypto::{KyberKeyPair, DilithiumKeyPair, Blake3Hasher};
...
mod crypto_integration_tests {
    ...
    async fn test_vault_kyber_integration() {
        ...
        let encapsulated = alice_vault.encapsulate_vault_key(&bob_keypair.public()).unwrap();
        
        // Bob decapsulates the vault key
        let shared_key = bob_vault.decapsulate_vault_key(
            &encapsulated,
            &bob_keypair
        ).unwrap();
        
        // Verify Bob can now access Alice's vault with the shared key
        let alice_vault_copy = Vault::open_with_key(
            vault_path.to_str().unwrap(),
            &shared_key
        ).unwrap();
        
        assert_eq!(alice_vault.vault_id(), alice_vault_copy.vault_id(),
...
mod dag_integration_tests {
    ...
    async fn test_vault_dag_structure() {
        ...
        vault.create_dag_node("root/personal", NodeType::Category).unwrap();
        vault.create_dag_node("root/work", NodeType::Category).unwrap();
        
        // Add secrets as DAG nodes
        vault.add_secret_as_dag_node(
            "email/gmail",
            "user@gmail.com",
            Some("pass"),
            vec!["root/personal"]
    ...
    async fn test_vault_dag_traversal() {
        ...
        vault.create_dag_node("apps/dev", NodeType::Category).unwrap();
        vault.create_dag_node("apps/prod", NodeType::Category).unwrap();
        vault.create_dag_node("shared", NodeType::Category).unwrap();
        
        // Add interconnected secrets
        vault.add_secret_as_dag_node("db/dev", "dev_user", Some("pass"), vec!["apps/dev"]).unwrap();
        vault.add_secret_as_dag_node("db/prod", "prod_user", Some("pass"), vec!["apps/prod"]).unwrap();
        vault.add_secret_as_dag_node("api/key", "api", Some("key"), vec!["apps/dev", "apps/prod", "shared"]).unwrap();
    ...
    async fn test_vault_dag_consensus() {
        ...
        let vaults: Vec<_> = vault_paths.iter()
            .map(|p| Arc::new(RwLock::new(
        ...
        // Add secret to first vault
        {
            ...
            vault0.add_secret("distributed/secret", "user", Some("pass")).unwrap();
        }
        ...
        for i in 1..3 {
            ...
            let updates = vault0.get_dag_updates_since(vault_i.last_sync_point()).unwrap();
...
mod network_integration_tests {
    use super::*;
    use qudag_network::{PeerManager, NetworkConfig};
    ...
    async fn test_vault_p2p_sync() {
        ...
        let vault1 = Vault::create_with_p2p(
            vault1_path.to_str().unwrap(),
            "Password1",
            peer1.clone()
        ).unwrap();
        
        let vault2_path = temp_dir.path().join("vault2.qdag");
        let vault2 = Vault::create_with_p2p(
            vault2_path.to_str().unwrap(),
            "Password2",
            peer2.clone()
        ).unwrap();
        
        // Add secret to vault1
        vault1.add_secret("sync/test", "user", Some("password")).unwrap();
    ...
    async fn test_vault_onion_routing() {
        ...
        let anonymous_share = vault.share_anonymously(
            &recipient_pubkey,
            OnionRoutingConfig {
                hops: 3,
                exit_node: None,
        ...
        assert!(anonymous_share.routing_proof.len() >= 3,
        ...
        for hop in &anonymous_share.routing_proof {
            assert!(hop.encrypted_layer.len() > 100,
...
mod performance_tests {
    ...
    async fn test_vault_dag_scalability() {
        ...
        // Create categories
        for i in 0..100 {
            vault.create_dag_node(&format!("category_{}", i), NodeType::Category).unwrap();
        }
        ...
        assert!(traverse_time < Duration::from_millis(100),
                "DAG traversal too slow: {:?}", traverse_time);
    }
}