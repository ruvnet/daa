...
async fn test_task_allocation() {
    ...
    let allocation = coordinator
        .allocate_training_task("model-001")
        .await
        .unwrap();
...
async fn test_consensus_mechanism() {
    ...
    coordinator
        .submit_vote(proposal_id, "node-000", true)
        .await
        .unwrap();
    coordinator
        .submit_vote(proposal_id, "node-001", true)
        .await
        .unwrap();
    coordinator
        .submit_vote(proposal_id, "node-002", true)
        .await
        .unwrap();
    coordinator
        .submit_vote(proposal_id, "node-003", false)
        .await
        .unwrap();
...
async fn test_daa_agent_integration() {
    ...
    let agent = coordinator
        .create_governance_agent("governance-001")
        .await
        .unwrap();
